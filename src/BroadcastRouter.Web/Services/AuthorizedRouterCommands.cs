...
{
    public async Task ExecuteAsync(string action, string? sourceId = null, string? portId = null,
        CancellationToken cancellationToken = default)
    {
        var principal = (await authenticationStateProvider.GetAuthenticationStateAsync()).User;
        OperatorAuthorization.EnsureAdministrator(principal);
        await coordinator.CommandAsync(action, sourceId, portId, cancellationToken);
    }
}

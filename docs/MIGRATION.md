...
4. Rescan real hardware and confirm each supported output reports a Blackmagic persistent hardware ID. Give each physical card an operator name such as `Studio input card`, then label its connectors once. Selectors will show `Studio input card / Input 1`. Create fixed/locked routes for batch files that represented permanent wiring and use rules/groups for dynamic routes. Legacy FFmpeg-hash mappings are migrated during a controlled host start; do not delete the pre-migration backup until connector verification passes.

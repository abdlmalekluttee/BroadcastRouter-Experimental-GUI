...
Version 1.5.13 requires no settings or database migration. It retains the saved-route retry behavior from 1.5.12 but removes the generic `rw_timeout` argument rejected by the production FFmpeg build. Normal-rate sources now use rapid duplicate-frame burst detection in the 250 ms supervision path; low-frame-rate sources retain the duration-based detector. Repeat the same controlled reset and reject the release if the route becomes `WaitingForStream`, repeatedly exits at input open, or waits for the next discovery cycle.

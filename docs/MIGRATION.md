...
Version 1.5.12 requires no settings or database migration. It applies `Routing.InputReadTimeoutMilliseconds` to both RTSP connection operations and established network reads, retains a saved route in standby-backed retry when discovery reports a transient loss, and gives the replacement process a bounded first-frame grace. Repeat the same controlled reset and reject the release if the route becomes `WaitingForStream` or waits for the next discovery cycle.

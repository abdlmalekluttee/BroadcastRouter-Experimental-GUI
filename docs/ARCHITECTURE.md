...
3. Render a validated RTSP template and run bounded, concurrency-limited FFprobe validation. Normal video requires decoded/read video frames. An audio-led input is eligible only when FFprobe counts real audio packets or frames; codec metadata alone remains insufficient. When the quick probe sees live audio plus an H.264 track but only sparse frames, one rate-limited 12-second confirmation waits for a long-GOP keyframe. Sustained extended evidence restores decoded-video mode immediately; ordinary quick observations retain the existing two-observation hysteresis.
...
9. Parse `-progress pipe:1`, drain stderr, enforce a first-progress deadline, detect later progress stalls, and detect live-video decoder sessions whose advancing output is almost entirely duplicated frames. RTSP control-session desynchronization is captured at stderr ingestion time because generated black can otherwise keep output progress alive after the real audio input has failed. Recreate only the affected owned process, retain its saved output reservation, classify exits, and retry with capped backoff plus jitter and an optional attempt limit. Audio-led generated-black routes are excluded from duplicate-frame detection. Process exits are observed before reconciliation so a failed start cannot be hidden by immediate recovery.

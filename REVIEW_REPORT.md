...
Resulting release version: `1.3.2`
...
| BR-026 | High | Audio-led media admission | `Infrastructure/FfprobeStreamProbe.cs`; `Infrastructure/FfmpegCommandBuilder.cs`; `Web/Services/RouterCoordinator.cs` | A publisher with continuous audio and only sparse still/black video opened successfully, but the two-second probe counted no video frames. The coordinator labeled every opened/no-video-frame result `UnsupportedMedia`, even though FFmpeg received the input. An isolated later frame could also revert the source to normal-video mode. | **Fixed in 1.2.11.** FFprobe counts audio packets/frames, requires sustained video cadence before overriding verified audio, and retains audio-led mode for the active publisher observation. Playout generates continuous preset-matched black video with 48 kHz stereo PCM live audio; `-shortest` prevents black output from outliving source audio. Regressions cover sparse-video/audio-only admission, isolated frames, sticky readiness, fail-closed metadata rejection, route arguments, and preview. |
...
Baseline: 37/37 tests. Version 1.2.2: 53/53. Version 1.2.3: 55/55. Version 1.2.4: 57/57. Version 1.2.5: 59/59. Version 1.2.6: 60/60. Version 1.2.8: 62/62. Version 1.2.9: 64/64. Version 1.2.10: 68/68. Version 1.2.11: 70/70. Versions 1.3.0 through 1.3.2: 75/75 tests.
